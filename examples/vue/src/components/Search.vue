...
<template>
  <form @submit.prevent="doSearchSubmit" class="search-field">
    ...
    <input
      class="block px-2 text-gray-500 text-sm w-16 sm:w-24 md:w-32 h-full"
      placeholder="Search"
      type="text"
  ...
  <div class="quick-search" :class="{ open: quickSearchStore.quickSearchOpen }">
    <div
      class="text-center"
      v-if="
        !quickSearchStore.products.length &&
        !quickSearchStore.trendingProducts.length
      "
    >
      Loading results...
    </div>
...
<style scoped>
.search-field {
  @apply relative bg-white border h-7 w-24 md:w-40 ml-auto mr-3;
}
.quick-search {
  top: 50px;
  right: 10%;
